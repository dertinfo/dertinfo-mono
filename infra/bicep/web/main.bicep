/*
Web workload: Azure Static Web App (Free) when prerequisitesExist.
Deploy: az deployment group create --resource-group rg-<env>-dertinfo-web-uks --template-file main.bicep --parameters main.<env>.bicepparam
...
@description('Custom hostname to bind after DNS exists (e.g. dev.dertinfo.co.uk). Empty until a name is chosen.')
...
// - API App Service app-<env>-dertinfo-api-uks (hosted API the SPA calls)
@description('When true, deploy the Static Web App. When false, succeed with no web resources.')
...
module staticSite 'br/public:avm/res/web/static-site:0.9.0' = if (prerequisitesExist) {
  name: 'avm-swa-web-${environmentTag}'
  params: {
