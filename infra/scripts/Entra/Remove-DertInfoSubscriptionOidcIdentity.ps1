<#
.SYNOPSIS
  Remove one Entra app registration used for subscription-scope GitHub Actions OIDC, including federated credentials and its service principal.

.DESCRIPTION
  Given the app (client) id:
  - Deletes every federated identity credential on the app
  - Deletes Azure role assignments for the service principal (if present)
  ...
  As an operator I run this script to delete one subscription OIDC app because I want that privileged identity gone.
...
.EXAMPLE
  .\Entra\Remove-DertInfoSubscriptionOidcIdentity.ps1 -ClientId '11111111-2222-3333-4444-555555555555'

.EXAMPLE
  .\Entra\Remove-DertInfoSubscriptionOidcIdentity.ps1 -ClientId '11111111-2222-3333-4444-555555555555' -Force
#>
...
function Invoke-Az {
  ...
  # az.cmd writes errors to stderr; with $ErrorActionPreference Stop that becomes a terminating NativeCommandError.
...
function Get-RoleAssignmentsForSp {
  ...
  # Do not use --all: Azure CLI rejects it when a default resource group/scope is configured.
  $subIds = @(
...
Write-Host "=== Removing OIDC identity $ClientId ===" -ForegroundColor Cyan
