...
**Development first-pass status (2026-09-19):** steps 1–6 are done. Both custom hosts resolve to the SWAs (placeholder start page until Src CD). GitHub Environment `development` has CALLBACKURL vars, `AZURE_WEBAPP_API_RESOURCENAME`, and SWA deploy tokens. App Configuration has `WebClient:Auth0:CallbackUrl` = `https://dev.dertinfo.co.uk`, `PwaClient:Auth0:CallbackUrl` = `https://app-dev.dertinfo.co.uk`, `Cors:AllowedOrigins` = those two hosts; API restarted. Remaining: step 7 (Web + App Src CD `dev-only`) then sign-in smoke test. Custom-domain bind needed the subscription role in [2026-09-19-001](../../operations/changelogs/2026-09-19-001-swa-operation-status-rbac.md).
