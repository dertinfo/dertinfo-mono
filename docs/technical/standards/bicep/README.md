...
- Web and app SPs also get a **subscription-scoped** custom role (`dertinfo-swa-operation-status-read-<env>`) with only `Microsoft.Web/locations/staticSitesOperationStatuses/read`. Custom-domain bind is async; ARM polls that action at `/subscriptions/…/providers/Microsoft.Web/locations/<region>/staticSitesOperationStatuses/…`, which RG Contributor cannot cover. Do not grant those SPs subscription Contributor.
