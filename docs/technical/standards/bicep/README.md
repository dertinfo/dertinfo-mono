...
- The Flex Function App uses raw `Microsoft.Web/serverfarms` + `Microsoft.Web/sites@2024-04-01` (`functionAppConfig`). Do not use AVM `web/site` 0.24.0 for Flex. Host storage still uses AVM `storage-account` 0.33.0. Hosted connections are identity-based (`AzureWebJobsStorage__accountName` / `StorageConnection__Images__accountName` — Linux Flex does not allow `:` in app setting names; `__` maps to `:` for the `StorageConnection:Images` blob trigger). Event Grid webhooks still use the Functions `blobs_extension` host key.
