...
- Resource providers for workload types are `resourceProvidersToRegister` in [`main.shared.bicepparam`](../../../infra/bicep/subscription/main.shared.bicepparam). Subscription CD (and [`Register-DertInfoResourceProviders.ps1`](../../../infra/scripts/Register-DertInfoResourceProviders.ps1) for break-glass) registers them. ARM has no PUT for registration; RG-scoped workload identities cannot register providers. Do not add a `Microsoft.Resources/providers` resource — that compiles but the API is POST `…/register` only. Keep the bash array in subscription CD and the script list in sync with that param.
