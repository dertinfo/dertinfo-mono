...
- Config Bicep deploys App Configuration Key Vault **references** for `auth0-managementclientsecret`, `az-storage-accountkey`, `mailgun-apikey`, and `sendgrid-apikey`. Values are set with [`New-DertInfoConfigKeyVaultSecrets.ps1`](../../../infra/scripts/New-DertInfoConfigKeyVaultSecrets.ps1), not in Bicep.
